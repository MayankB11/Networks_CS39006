...
void* dummy_tcp::udp_recieve(void* arg)
{
	...
	while(1){
		...
		if(drop<50  && r < 0.8 && packet->header.seq != 0)
		{
			cout << "Dropped" << endl;
			drop++;
			continue;
...
void* dummy_tcp::reciever_buffer_handler(packet_t *packet)
{
	if(packet->header.seq == excepted_seq_no)
		{
			while(packet->header.length > (MAX_BUFFERSIZE-reciever_data_buffer.size()));
			//acquire lock
			sem_wait(&recieve);
			for(int i=0; i<packet->header.length; i++)
			{	
			...
			while(reciever_packet_queue.find(excepted_seq_no)!=reciever_packet_queue.end())
			{
				...
				while(tmp.header.length > (MAX_BUFFERSIZE-reciever_data_buffer.size()));
				//acquire lock
				sem_wait(&recieve);
...
void* dummy_tcp::flow_control()
{
	...
	else if((this->base_ptr - this->cur_ptr) > swnd || this->timeout == 1){
		// cout << "Here" << endl;
